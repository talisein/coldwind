...
void Derp::Parser::parse_thread(const Derp::Request& request) {
	...
	if (!setup_curl( request_.getUrl() )) {
		std::cerr << "Error: Couldn't setup curl for fetching "
		          << request_.getUrl() << std::endl;
	...
	if (code != CURLE_OK) {
		...
		case CURLE_HTTP_RETURNED_ERROR:
			...
			if (responseCode == 404) {
				...
				std::cerr << "Error: Curl ran into an HTTP " << responseCode
				          << " error.";
				signal_fetching_error();
			}
			break;
...
static bool is_code_ok(CURLcode code, std::string str) {
	if (code != CURLE_OK) {
		std::cerr << "Error: While setting up curl to fetch the thread in "
		          << str << " : " << curl_easy_strerror(code) << std::endl;
		return false;
	}
	return true;
}
